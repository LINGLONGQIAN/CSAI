...
args.data_path = root_data_path + args.dataset + '/data_nan.pkl'
args.label_path = root_data_path + args.dataset + '/label.pkl'
...
elif args.dataset == 'mimic_89f':
    args.vae_hiddens = [89, 128, 32, 16]
    attributes = ['Gastric Gastric Tube','Stool Out Stool','Urine Out Incontinent','Ultrafiltrate Ultrafiltrate','Foley','Void','Condom Cath','Fecal Bag','Ostomy (output)','Chest Tube #1','Chest Tube #2','Jackson Pratt #1',
        'OR EBL','Pre-Admission','TF Residual','Albumin 5%','Dextrose 5%','Fresh Frozen Plasma','Lorazepam (Ativan)','Calcium Gluconate','Midazolam (Versed)','Phenylephrine','Furosemide (Lasix)','Hydralazine','Norepinephrine',
        'Magnesium Sulfate_input','Nitroglycerin','Insulin - Regular','Insulin - Glargine','Insulin - Humalog','Heparin Sodium','Morphine Sulfate','Potassium Chloride_input','Packed Red Blood Cells','Gastric Meds','D5 1/2NS',
        'LR','K Phos','Solution','Sterile Water','Metoprolol','Piggyback','OR Crystalloid Intake','OR Cell Saver Intake','PO Intake','GT Flush','KCL (Bolus)','Magnesium Sulfate (Bolus)','HEMATOCRIT','WHITE BLOOD CELLS','PLATELET COUNT',
        'HEMOGLOBIN','MCHC','MCH','MCV','RED BLOOD CELLS','RDW','POTASSIUM','SODIUM','CHLORIDE','BICARBONATE','ANION GAP','UREA NITROGEN','CREATININE','GLUCOSE','MAGNESIUM','CALCIUM, TOTAL','PHOSPHATE','INR(PT)','PT','PTT','LYMPHOCYTES',
        'MONOCYTES','NEUTROPHILS','BASOPHILS','EOSINOPHILS','BILIRUBIN, TOTAL','PH','BASE EXCESS','CALCULATED TOTAL CO2','PO2','PCO2','SPECIFIC GRAVITY','LACTATE','ALANINE AMINOTRANSFERASE (ALT)','ASPARATE AMINOTRANSFERASE (AST)',
        'ALKALINE PHOSPHATASE','ALBUMIN','Pantoprazole']
...
elif args.dataset == 'physionet_all':
    args.vae_hiddens = [35, 64, 24, 10]
    attributes = ['DiasABP', 'HR', 'Na', 'Lactate', 'NIDiasABP', 'PaO2', 'WBC', 'pH', 'Albumin', 'ALT', 'Glucose', 'SaO2', 'Temp', 'AST', 'Bilirubin', 'HCO3', 'BUN', 'RespRate', 'Mg', 'HCT', 'SysABP', 'FiO2', 'K', 'GCS',
        'Cholesterol', 'NISysABP', 'TroponinT', 'MAP', 'TroponinI', 'PaCO2', 'Platelets', 'Urine', 'NIMAP', 'Creatinine', 'ALP']
...
for fold in range(5):
    ...
    train_data, mean_set, std_set, intervals = normalize_new(data=train_data, mean=[], std=[], compute_intervals=True)
    valid_data, m, s = normalize_new(valid_data, mean_set, std_set)
    test_data, m, s = normalize_new(test_data, mean_set, std_set)
    ...
    if args.task == 'I':
        ...
        model_path = './{}/{}/{}/task_I/fold_{}/model_state/model_{}_best_valid_I_state_dict.pth'.format(args.model_path, args.dataset, args.pre_model,fold,fold)
        model = net(args=args, medians_df=intervals, get_y=True).to(args.device)
        model.load_state_dict(torch.load(model_path))
    elif args.task == 'pretrain_brits':
        pretrain_brits_path = './{}/{}/{}/task_I/fold_{}/model_state/model_{}_best_valid_I_state_dict.pth'.format(args.model_path, args.dataset, args.pre_model,fold,fold)
        pretrained_brits_state_dict = torch.load(pretrain_brits_path)
        model = net(args=args, medians_df=intervals, get_y=True).to(args.device)
        selected_state_dict = {k: v for k, v in pretrained_brits_state_dict.items() if k in model.state_dict() and model.state_dict()[k].shape == v.shape}
    ...
    for epoch in range(args.epoch):
        ...
        if args.task == 'I':
            if train_res['mae'] < train['mae']:
                ...
                get_polarfig(args, train_replacement_probabilities, train_missing_rates, train_res['feature_mae'], dir, fold, 'train', attributes)
            ...
            if valid_res['mae'] < valid['mae']:
                ...
                get_polarfig(args, train_replacement_probabilities, valid_missing_rates, valid_res['feature_mae'], dir, fold, 'valid', attributes)
            ...
            if test_res['mae'] < test['mae']:
                ...
                get_polarfig(args, train_replacement_probabilities, test_missing_rates, test_res['feature_mae'], dir, fold, 'test', attributes)
